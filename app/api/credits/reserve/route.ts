...
import { reserveServerCredits, CreditTransactionError } from "../../../../lib/db/repositories/credits.js";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await withDbRetry(() => reserveServerCredits(body, resolveWorkspaceId(request)));
    return json(result);
  } catch (error) {
