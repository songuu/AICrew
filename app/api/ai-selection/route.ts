...
export async function GET(request: Request) {
  ...
  try {
    const selection = await loadDocument(DOC_TYPES.aiSelection, resolveWorkspaceId(request));
...
export async function PUT(request: Request) {
  ...
  try {
    await saveDocument(DOC_TYPES.aiSelection, body?.selection ?? body, resolveWorkspaceId(request));
    return json({ ok: true });
  } catch {
    return json({ error: INTERNAL_ERROR_MESSAGE }, 500);
  }
