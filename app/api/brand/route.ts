...
export async function GET(request: Request) {
  ...
  try {
    const brand = await loadDocument(DOC_TYPES.brand, resolveWorkspaceId(request));
    return json({ brand });
  } catch {
...
export async function PUT(request: Request) {
  ...
  try {
    await saveDocument(DOC_TYPES.brand, body?.brand ?? body, resolveWorkspaceId(request));
    return json({ ok: true });
  } catch {
    return json({ error: INTERNAL_ERROR_MESSAGE }, 500);
  }
