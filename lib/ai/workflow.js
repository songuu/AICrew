...
const HOOK_FRAMEWORKS = {
  ...
  // —— 获客专属框架（供小红书获客类 skill 的 skill.hookPatterns 选用）——
  诱饵留白钩: "价值前置（晒部分成果证明我有）+ 留白（完整的太长这里放不下）+ 唯一低门槛行动指令（扣 1 / 私信关键词领），同时拉评论权重与收口线索",
  截流神评钩: "在别人爆文评论区给真实价值（干货补充 / 反差观点）+ 人设背书（亮身份/经验）+ 主页承接暗示（绝不放联系方式）",
  痛点筛选钩: "精准点名某类人的具体痛点 + 圈定人群（只有 XX 人懂）+ 给出对应解法入口，过滤泛粉只留高意向线索",
  // —— 抖音获客专属框架（供抖音视频/直播类 skill 的 skill.hookPatterns 选用）——
  完播钩: "前 3 秒抛冲突/反差/利益前置压住跳出 → 中段每 5 秒一个信息点不注水 → 断点埋好奇缺口拖完播 → 结尾给关注理由，服务赛马进池门槛（完播率≈40%）",
  直播憋单钩: "先别走 → 报老粉福利 → 报库存营造稀缺 → 留价格悬念 → 扣『要』拉互动同时筛意向；合规版用内容驱动（故事/小剧场）替代强行憋库存",
  直播逼单钩: "重复核心卖点 → 打消疑虑（对比/退换承诺）→ 倒计时制造紧迫（3·2·1 上链接）→ 报剩余库存营造畅销 → 价格锚点对比 → 临门一脚指令",
  // —— 视频号获客专属框架（供视频号社交裂变/直播类 skill 的 skill.hookPatterns 选用）——
  社交转发钩: "利他理由前置（替朋友说出想说的话/帮 TA 表态）+ 一句低门槛转发指令（转给那个需要的朋友）+ 借熟人关系链破圈，把点赞/转发转成『朋友在看』的二级三级曝光",
  直播预约钩: "开播价值/福利前置（预约才有的专属）+ 稀缺名额/限时 + 一键预约指令，把社交流量沉成开播前的『服务通知』强提醒，撬动起始公域流量",
  // —— 互动引导框架（供小红书首评/互动类 skill 的 skill.hookPatterns 选用）——
  互动提问钩: "在自己笔记的置顶首评抛一个低门槛、二选一或填空式问题（你们是 A 还是 B / 还想看哪个），让读者用一句话就能回，把评论欲转成真实评论与点赞，撬动互动权重而非硬广"
};
...
function renderHookGuidance(brief, preset, skill) {
  // skill 可声明专属 hookPatterns 覆盖平台默认：获客类技能（评论截流 / 引流钩子 / 私信 SOP / 群聊）
  // 用截流神评钩 / 诱饵留白钩 / 痛点筛选钩等获客框架，而非平台通用框架。
  // 无声明则回退平台 preset.hookPatterns——所有既有技能行为逐字不变（向后兼容）。
  const patterns = Array.isArray(skill?.hookPatterns) && skill.hookPatterns.length
    ? skill.hookPatterns
    : (Array.isArray(preset?.hookPatterns) ? preset.hookPatterns : []);
