...
import { createDeferredArtifact, createFailedArtifact, createReadyArtifact, upsertArtifact, variantImageRefKey } from "../artifacts.js";
  ...
  const results = await Promise.all(
    list.map(async (variant, index) => {
      ...
      try {
        ...
        return { index, variantId: variant?.id, imageUrl: null, error: error instanceof Error ? error.message : String(error) };
      }
    })
  );
