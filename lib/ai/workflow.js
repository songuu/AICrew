...
async function executeCopyAgent(context) {
  ...
  const failedCount = copyResults.filter(result => result.error).length;
  if (failedCount === sourceVariants.length && sourceVariants.length > 0) {
    return failedExecution("copy", "text", `copy generation failed for ${failedCount}/${sourceVariants.length} variants`, copyResults.find(result => result.error)?.error);
  }
...
export async function runCreativeWorkflowWithAI({ brief, skillId, skill, brandKit = defaultBrandKit, aiConfig, enabledModes, signal, fetchImpl, variantCount } = {}) {
  ...
  try {
    ...
    const runAgents = imageTotalFailure
      ? enrichedAgents.map(agent => (agent.id === "visual"
          ? { ...agent, status: TASK_STATUS.failed, progress: 0, error: sanitizeArtifactError(context.imageErrors[0].error), finishedAt: new Date().toISOString() }
          : agent))
      : enrichedAgents;
    return {
      ...base,
      status: imageTotalFailure ? TASK_STATUS.failed : base.status,
      variants: context.variants,
      agents: runAgents,
      aiMeta: {
