...
export async function runCreativeWorkflowWithAI({ brief, skillId, skill, brandKit = defaultBrandKit, aiConfig, enabledModes, signal, fetchImpl, variantCount } = {}) {
  ...
  try {
    ...
    const imageTotalFailure = imageEnabled && imageErrors.length > 0 && imageAppliedCount === 0;
    const runAgents = imageTotalFailure
      ? enrichedAgents.map(agent => (agent.id === "visual"
          ? { ...agent, status: TASK_STATUS.failed, progress: 0, error: sanitizeArtifactError(imageErrors[0].error), finishedAt: new Date().toISOString() }
          : agent))
      : enrichedAgents;
    return {
      ...base,
    ...
    return { ...base, status: TASK_STATUS.failed, aiMeta: { used: false, error: sanitizeArtifactError(error) } };
  }
}
