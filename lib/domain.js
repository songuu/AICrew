...
export const skillGroups = [
  ...
  { id: "channels", name: "视频号获客", desc: "社交裂变、直播预约、公私域闭环、企微承接" },
...
export const skills = [
  ...
  // —— 小红书内容层缺口补全（对标小鸡AI 全生态：图片配文 / 氛围评论 / 定时批量发布）——
  // 这三项是小鸡AI 生态里 AICrew 内容层尚缺的部分；自动化层（采集/批量发布/自动评论点赞）
  // 既踩平台红线又是静态站架构外，故只取「内容产物」一半：看图配文 / 自己笔记的首评引导 / 发布前自检清单。
  {
    // 图片智能配文：从产品图/实拍图反推卖点 → 生成小红书风格标题+正文+话题。
    // 对标小鸡AI「图片智能配文」，是内容层能力（人工挑图后用），不碰自动发布。
    id: "rednote_image_caption_v1",
    name: "图片智能配文",
    icon: "🖼️",
    group: "rednote",
    featured: false,
    ...
    platform: "小红书",
    rednoteStage: "content_production",
    recommendTags: ["图片配文", "配图文案", "看图写文", "图片文案", "图配文", "首图文案"],
    estimatedCredits: 70,
    formats: ["图片卖点解读", "小红书标题", "图文正文", "话题标签"],
    agents: ["brief", "strategy", "persona", "hook", "copy", "seo", "qa", "export"],
    hookPatterns: ["具体数字", "好奇缺口", "痛点筛选钩", "真实自白"],
    promise: "从产品图/实拍图反推卖点，生成小红书风格的标题、正文与话题标签，让一组图片直接成稿",
    bestFor: "有大量产品图/实拍图但不会写文案的电商、买手、本地商家和素人博主",
    palette: ["#ffb86b", "#ff9ecf", "#a78bfa"]
  },
  {
    // 首评氛围与互动引导：为「自己的」笔记写置顶首评 + 互动提问，撬动评论/点赞/收藏。
    // 对标小鸡AI「AI 氛围评论 + 自动点赞」，但只产自己笔记的引导内容，绝不自动评论/点赞他人（红线）。
    id: "rednote_first_comment_v1",
    name: "首评氛围与互动引导",
    icon: "💯",
    group: "rednote",
    featured: false,
    ...
    platform: "小红书",
    rednoteStage: "content_production",
    recommendTags: ["首评", "氛围评论", "互动引导", "置顶评论", "评论钩子", "点赞收藏引导"],
    estimatedCredits: 60,
    formats: ["置顶首评文案", "互动提问钩子", "评论区话题引导", "点赞收藏 CTA"],
    agents: ["brief", "strategy", "persona", "hook", "copy", "qa", "export"],
    hookPatterns: ["互动提问钩", "好奇缺口", "真实自白", "社会证明"],
    promise: "为自己的笔记生成置顶首评、互动提问和点赞收藏引导话术，自然撬动评论区氛围与互动数据",
    bestFor: "想提升自己笔记互动率、用首评和提问引导评论的博主、品牌号和运营",
    palette: ["#ff7a90", "#ffd166", "#5eead4"]
  },
  {
    // 发布优化清单：发布前最后一公里——标题/封面终检 + 最佳发布时段 + 话题/定位/合规自检。
    // 对标小鸡AI「定时/批量发布」，但只产「发布前内容自检清单」，不代为自动发布（红线 + 静态站做不到）。
    id: "rednote_publish_optimizer_v1",
    name: "发布优化清单",
    icon: "🚀",
    group: "rednote",
    featured: false,
    ...
    platform: "小红书",
    rednoteStage: "content_production",
    recommendTags: ["发布优化", "发布前检查", "发布清单", "最佳发布时间", "发布自检", "标题封面终检"],
    estimatedCredits: 55,
    formats: ["发布前自检清单", "最佳发布时段建议", "标题/封面终检", "话题与定位补全"],
    agents: ["brief", "strategy", "seo", "copy", "qa", "export"],
    promise: "发布前最后一公里自检：标题封面终检、最佳发布时段、话题定位与合规一次过清单，降低首发翻车",
    bestFor: "笔记写完但不确定何时发、发布前想要一份终检清单的运营、博主和商家",
    palette: ["#8bd3ff", "#5eead4", "#ffd166"]
  },
  ...
  // —— 视频号获客生态（参照视频号商业化矩阵 + 微信公私域闭环研究）——
  // 镜像 rednote/douyin：group="channels" + platform="视频号" + channelsStage，经 channelsPromotionSkills() 聚合成「视频号获客」tab。
  // 视频号灵魂=微信生态公私域闭环 + 社交关系链裂变；漏斗多 社交裂变/小店带货/生态联动 三个微信特有环节。
  // 边界同前：只产内容/话术/脚本/SOP/策略，不做账号自动化/自动私信/SCRM/爬虫；尤其规避微信诱导分享与硬导流红线。
  {
    id: "channels_account_positioning_v1",
    name: "视频号账号定位诊断",
    icon: "🧭",
    group: "channels",
    featured: true,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "positioning",
    recommendTags: ["账号定位", "人设差异化", "人群画像", "卖点诊断", "主页优化", "赛道选择"],
    estimatedCredits: 90,
    formats: ["账号定位诊断报告", "人设差异化方案", "主页/简介/合集结构优化清单", "卖点-人群匹配表"],
    agents: ["brief", "trend", "strategy", "persona", "seo", "qa", "export"],
    hookPatterns: ["反共识", "好奇缺口", "痛点筛选钩", "具体数字"],
    promise: "诊断视频号账号定位与人设差异化，结合微信生态(公众号/企微)给出主页、合集和卖点-人群匹配的改版清单",
    bestFor: "刚起号或定位模糊的视频号品牌号、个人 IP、本地服务商",
    palette: ["#07C160", "#161823", "#95EC69"]
  },
  {
    id: "channels_search_seo_v1",
    name: "视频号搜一搜SEO卡位",
    icon: "🔍",
    group: "channels",
    featured: true,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "search_seo",
    recommendTags: ["搜一搜", "视频号SEO", "关键词布局", "话题标签", "标题卡位", "埋词"],
    estimatedCredits: 85,
    formats: ["核心词/长尾词/场景词/疑问词分层词库", "搜一搜/视频号标题卡位模板", "话题(#)标签矩阵", "简介/口播/字幕埋词方案"],
    agents: ["brief", "trend", "strategy", "seo", "persona", "copy", "qa", "export"],
    hookPatterns: ["具体数字", "好奇缺口", "痛点筛选钩"],
    promise: "围绕业务词产出分层关键词库、卡位标题、话题标签与埋词方案，抢占微信搜一搜与视频号搜索自然流量",
    bestFor: "想吃微信搜一搜/视频号搜索自然流量的本地商家、知识 IP、B 端获客",
    palette: ["#07C160", "#161823", "#6BCB77"]
  },
  {
    // 视频链路：社交推荐友好的完播结构引流短视频，引导加企微/关注公众号/进群。
    id: "channels_lead_gen_shortvideo_v1",
    name: "视频号引流短视频脚本",
    icon: "🎬",
    group: "channels",
    featured: true,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "content_engine",
    recommendTags: ["引流短视频", "完播结构", "钩子开场", "社交推荐", "加企微引导", "公众号引导"],
    estimatedCredits: 110,
    formats: ["9:16 口播/剧情脚本", "黄金3秒钩子开场库", "分镜+字幕/BGM建议", "加企微/关注公众号/进群引导 CTA"],
    agents: ["brief", "trend", "hook", "script", "storyboard", "video", "copy", "qa", "export"],
    hookPatterns: ["完播钩", "痛点开场", "好奇缺口", "社会证明"],
    promise: "把卖点拆成钩子→价值→引导加企微/关注公众号/进群的视频号短视频脚本与分镜，按社交推荐友好的完播结构编排",
    bestFor: "靠视频号公域+社交推荐引流到企微/公众号/社群的品牌与获客型商家",
    palette: ["#07C160", "#161823", "#95EC69"]
  },
  {
    id: "channels_viral_rewrite_v1",
    name: "视频号爆款对标改写",
    icon: "♻️",
    group: "channels",
    featured: false,
    ...
    platform: "视频号",
    channelsStage: "content_engine",
    recommendTags: ["爆款改写", "对标拆解", "结构套用", "同结构原创", "二创", "选题复用"],
    estimatedCredits: 85,
    formats: ["爆款结构拆解(开头/节奏/钩子/CTA)", "钩子-节奏迁移映射", "同结构原创脚本", "批量改写选题清单"],
    agents: ["brief", "trend", "strategy", "hook", "persona", "copy", "qa", "export"],
    hookPatterns: ["反共识", "好奇缺口", "具体数字", "真实自白"],
    promise: "拆解一条视频号对标爆款的开头-节奏-钩子-CTA 结构，套用到自己业务改写成不撞车的同结构原创脚本",
    bestFor: "贴入对标视频号爆款想快速产出同结构原创内容的运营与矩阵号",
    palette: ["#07C160", "#1AAD19", "#95EC69"]
  },
  {
    id: "channels_matrix_warmup_v1",
    name: "视频号矩阵起号养号计划",
    icon: "🧩",
    group: "channels",
    featured: false,
    ...
    platform: "视频号",
    channelsStage: "content_engine",
    recommendTags: ["矩阵号", "起号养号", "多账号人设", "30天内容日历", "选题分发", "防同质化"],
    estimatedCredits: 95,
    formats: ["多账号差异化人设", "30天起号内容日历", "混剪改写规则", "矩阵选题分发表"],
    agents: ["brief", "strategy", "persona", "trend", "script", "seo", "qa", "export"],
    promise: "生成多账号差异化人设、30 天起号内容计划与混剪改写规则，只做内容计划层不碰批量发布与账号自动化",
    bestFor: "做视频号矩阵号但要规避同质化的操盘手、本地连锁、MCN",
    palette: ["#07C160", "#161823", "#5AD8A6"]
  },
  {
    // 社交裂变是视频号灵魂机制——接入社交转发钩，把点赞/转发转成『朋友在看』的二级三级曝光。
    id: "channels_social_fission_pack_v1",
    name: "视频号社交裂变玩法包",
    icon: "🔗",
    group: "channels",
    featured: true,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "social_fission",
    recommendTags: ["社交裂变", "朋友圈转发", "点赞助力", "好友裂变", "直播预约裂变", "裂变海报"],
    estimatedCredits: 85,
    formats: ["转发朋友圈话术+文案模板", "点赞助力/好友裂变诱饵设计", "直播预约裂变话术(分享得福利)", "社群裂变海报文案+任务规则"],
    agents: ["brief", "strategy", "hook", "persona", "copy", "qa", "export"],
    hookPatterns: ["社交转发钩", "诱饵留白钩", "社会证明", "痛点筛选钩"],
    promise: "产出转发朋友圈、点赞助力、好友裂变诱饵、直播预约裂变和社群裂变海报的全套话术与玩法规则，激活微信社交关系链",
    bestFor: "想用微信关系链做低成本裂变拉新的私域操盘手、活动运营",
    palette: ["#07C160", "#1AAD19", "#95EC69"]
  },
  {
    // 视频链路：直播全场脚本 SOP，接入直播预约钩（视频号直播靠预约撬动社交关系链强提醒）。
    id: "channels_live_acquisition_script_v1",
    name: "视频号直播获客脚本SOP",
    icon: "📡",
    group: "channels",
    featured: true,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "live_acquisition",
    recommendTags: ["直播脚本", "直播预约", "憋单逼单", "留资承接", "场控SOP", "开场留人"],
    estimatedCredits: 125,
    formats: ["直播预约引流+预热海报文案", "分段脚本(开场留人/价值/留资/憋单/逼单)", "加企微/进群留资承接话术", "场控SOP时间轴+互动节点(点赞/评论/分享)"],
    agents: ["brief", "strategy", "hook", "script", "storyboard", "video", "copy", "qa", "export"],
    hookPatterns: ["直播预约钩", "直播憋单钩", "直播逼单钩", "社会证明"],
    promise: "生成预约引流→开场留人→价值→引导加企微/进群留资→憋单逼单的视频号直播全场脚本与场控 SOP，憋单逼单含合规改写版",
    bestFor: "做视频号直播获客/带货的主播、场控、操盘手",
    palette: ["#07C160", "#161823", "#FAE03C"]
  },
  {
    // 视频链路：视频号小店/直播带货商品素材。
    id: "channels_shop_product_material_v1",
    name: "视频号小店带货素材包",
    icon: "🛍️",
    group: "channels",
    featured: false,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "wechat_commerce",
    recommendTags: ["视频号小店", "带货素材", "商品讲解", "过款话术", "商品卡", "橱窗"],
    estimatedCredits: 115,
    formats: ["商品讲解短视频脚本(9:16)", "直播带货过款话术", "商品卡/橱窗标题+卖点短句", "主图/封面卖点标注方向"],
    agents: ["brief", "trend", "strategy", "script", "storyboard", "visual", "video", "copy", "qa", "export"],
    hookPatterns: ["具体数字", "社会证明", "紧迫", "痛点开场"],
    promise: "围绕同一商品产出讲解短视频脚本、直播过款话术、商品卡卖点和主图标注方向，服务视频号小店与直播带货",
    bestFor: "做视频号小店/直播带货的商家、达人与带货团队",
    palette: ["#07C160", "#161823", "#FFB86B"]
  },
  {
    id: "channels_official_account_linkage_v1",
    name: "公众号×视频号联动涨粉",
    icon: "🔁",
    group: "channels",
    featured: false,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "ecosystem_linkage",
    recommendTags: ["公众号联动", "互导文案", "视频号挂载", "涨粉钩子", "关注引导", "生态联动"],
    estimatedCredits: 80,
    formats: ["公众号→视频号互导文案", "视频号→公众号挂载/引导话术", "涨粉钩子+关注理由文案", "图文挂载视频号的导流段落模板"],
    agents: ["brief", "strategy", "seo", "persona", "copy", "qa", "export"],
    hookPatterns: ["好奇缺口", "诱饵留白钩", "社会证明"],
    promise: "生成公众号与视频号双向互导文案、挂载引导和涨粉钩子，打通图文↔视频↔关注的微信生态流量闭环",
    bestFor: "同时运营公众号和视频号、想互相导流涨粉的品牌与个人 IP",
    palette: ["#07C160", "#1AAD19", "#95EC69"]
  },
  {
    id: "channels_lead_magnet_funnel_v1",
    name: "视频号引流诱饵留资漏斗",
    icon: "🪝",
    group: "channels",
    featured: true,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "lead_capture",
    recommendTags: ["引流诱饵", "线索留资", "资料包", "领取引导", "承接路径", "公私域承接"],
    estimatedCredits: 90,
    formats: ["钩子诱饵命名+资料包大纲", "视频内/评论区领取引导话术", "公众号/企微/小程序承接路径与话术", "承接路径漏斗图+各节点话术"],
    agents: ["brief", "strategy", "hook", "persona", "copy", "qa", "export"],
    hookPatterns: ["诱饵留白钩", "痛点筛选钩", "具体数字", "好奇缺口"],
    promise: "设计资料/福利诱饵命名、领取引导和公众号/企微/小程序承接路径话术，把视频号曝光合规收口成线索",
    bestFor: "用免费资料/福利换线索、需公众号/企微/小程序承接的教培、B 端、本地服务",
    palette: ["#07C160", "#161823", "#6BCB77"]
  },
  {
    // 横切守卫：微信诱导分享 + 私域硬导流红线校验改写（视频号合规绞杀严，这是差异化护城河）。
    id: "channels_compliance_guard_v1",
    name: "视频号导流合规红线校验改写",
    icon: "🛡️",
    group: "channels",
    featured: false,
    ...
    platform: "视频号",
    channelsStage: "social_fission",
    recommendTags: ["合规改写", "红线校验", "诱导分享", "违规词替换", "导流合规", "封号风险规避"],
    estimatedCredits: 70,
    formats: ["诱导分享/利诱关注红线对照表", "私域硬导流(留微信/二维码)违规词→替换词表", "裂变/导流话术合规改写版", "合规承接路径建议(一体号/公众号菜单/留资组件)"],
    agents: ["brief", "strategy", "copy", "qa", "export"],
    hookPatterns: ["真实自白", "社会证明"],
    promise: "把任意裂变/导流文案过一遍微信诱导分享与私域导流红线，自动标红违规表述并改写为合规版本，规避限流封号",
    bestFor: "做视频号裂变/导流但担心踩诱导分享与私域导流红线的私域操盘手、活动运营",
    palette: ["#07C160", "#161823", "#95EC69"]
  },
  {
    id: "channels_private_domain_handoff_v1",
    name: "企微社群私域承接SOP",
    icon: "🏠",
    group: "channels",
    featured: true,
    ...
    stage: "P1",
    platform: "视频号",
    channelsStage: "private_domain",
    recommendTags: ["企微承接", "社群运营", "欢迎语", "朋友圈剧本", "运营日历", "复购召回"],
    estimatedCredits: 90,
    formats: ["企微/社群欢迎语脚本树", "承接SOP(加微→破冰→给价值→留存)", "朋友圈剧本(人设/种草/成交/答疑)", "社群运营日历+复购召回话术"],
    agents: ["brief", "strategy", "persona", "hook", "copy", "qa", "export"],
    hookPatterns: ["诱饵留白钩", "真实自白", "社会证明", "痛点筛选钩"],
    promise: "搭建企微/社群欢迎语、承接 SOP、朋友圈剧本、运营日历和复购召回话术，把视频号线索养成长期复购",
    bestFor: "用企微+社群承接视频号线索、做长期私域复购的商家与运营团队",
    palette: ["#07C160", "#1AAD19", "#95EC69"]
  }
];
...
// —— 视频号获客体系：镜像 rednote/douyin，复用通用 recommendPromotionSkills 内核 ——
// 视频号灵魂是微信生态公私域闭环 + 社交关系链裂变，漏斗多了 社交裂变 / 小店带货 / 生态联动 三个微信特有环节。
export const channelsPromotionStages = [
  { id: "positioning", name: "定位诊断", desc: "账号定位与人设、人群画像、卖点诊断、主页/合集优化" },
  { id: "search_seo", name: "搜一搜卡位", desc: "微信搜一搜/视频号搜索关键词布局、标题/话题/简介/字幕埋词" },
  { id: "content_engine", name: "内容引流", desc: "完播友好短视频脚本、爆款对标改写、矩阵起号养号内容计划" },
  { id: "social_fission", name: "社交裂变", desc: "朋友圈转发、点赞助力、好友/社群裂变诱饵、导流合规校验" },
  { id: "live_acquisition", name: "直播获客", desc: "直播预约引流、开场留人/憋单/逼单/留资承接脚本与场控 SOP" },
  { id: "wechat_commerce", name: "小店带货", desc: "视频号小店商品讲解脚本、过款话术、商品卡/橱窗卖点" },
  { id: "ecosystem_linkage", name: "生态联动", desc: "公众号×视频号互导挂载、涨粉钩子、小程序承接联动" },
  { id: "lead_capture", name: "线索留资", desc: "引流诱饵设计、领取引导、公众号/企微/小程序承接路径话术" },
  { id: "private_domain", name: "私域承接", desc: "企微/社群欢迎语与 SOP、朋友圈剧本、运营日历、复购召回" }
];

export function channelsPromotionSkills() {
  return skills.filter(skill => skill.group === "channels" || skill.id.startsWith("channels_"));
}

export function recommendChannelsSkills(input = {}) {
  return recommendPromotionSkills({ skillset: channelsPromotionSkills(), stages: channelsPromotionStages, stageKey: "channelsStage", input });
}

// —— 平台获客体系注册表：把每个获客 group → {阶段集 / 技能集 / 阶段字段 / 推荐函数} 收敛到单点 ——
// 同时服务 skillsInGroup（分类列表）、promotionFunnelForGroup（UI 漏斗分组）、recommendForGroup（意图推荐），
// 避免「rednote/douyin/channels」三处散落判断；加一个平台获客漏斗只需在此登记一行。
const PROMOTION_REGISTRY = {
  rednote: { stages: rednotePromotionStages, skills: rednotePromotionSkills, stageKey: "rednoteStage", recommend: recommendRednoteSkills },
  douyin: { stages: douyinPromotionStages, skills: douyinPromotionSkills, stageKey: "douyinStage", recommend: recommendDouyinSkills },
  channels: { stages: channelsPromotionStages, skills: channelsPromotionSkills, stageKey: "channelsStage", recommend: recommendChannelsSkills }
};

// 该 group 是否为平台获客体系（带漏斗与推荐路由），用于 UI 决定是否走漏斗分组视图。
export function isPromotionGroup(groupId) {
  return Object.prototype.hasOwnProperty.call(PROMOTION_REGISTRY, groupId);
}

// 把某获客 group 的技能按漏斗阶段分组：返回 [{ stage, skills }]（阶段顺序＝漏斗顺序），非获客 group 返回 null。
// 供技能选择器在平台 tab 内渲染「获客漏斗」分段，驯服扁平长列表并可视化阶段结构。
export function promotionFunnelForGroup(groupId) {
  const system = PROMOTION_REGISTRY[groupId];
  if (!system) return null;
  const list = system.skills();
  return system.stages.map(stage => ({ stage, skills: list.filter(skill => skill[system.stageKey] === stage.id) }));
}

// 按 group 把用户意图（创意文本 + 可选阶段）路由到该平台最相关的技能；非获客 group 返回 []。
export function recommendForGroup(groupId, input = {}) {
  const system = PROMOTION_REGISTRY[groupId];
  return system ? system.recommend(input) : [];
}

// 按 group 取技能；「featured」/空 → 推荐 tab（featured 标记的技能）；获客 group → 经注册表取其完整体系。
...
export function skillsInGroup(groupId) {
  ...
  if (isPromotionGroup(groupId)) {
    return PROMOTION_REGISTRY[groupId].skills();
  }
...
export const platformPresets = [
  ...
  {
    // 视频号：微信生态唯一「公域算法 + 社交关系链」双引擎入口。灵魂是公私域闭环
    // （视频号↔公众号↔企微↔社群↔小程序↔朋友圈）+ 社交推荐裂变，先撬点赞转发再承接私域。
    id: "channels",
    name: "视频号",
    ratio: "9:16",
    hookSeconds: 3,
    tone: "微信熟人社交、社交推荐友好、信任优先弱硬推，先引导点赞转发撬动社交关系链，再承接关注公众号/加企微/进群的公私域闭环",
    lang: "zh",
    hookPatterns: ["社会证明", "真实自白", "完播钩", "诱饵留白钩"],
    copyRules: {
      hookMaxChars: 20,
      captionRange: [40, 500],
      emoji: "克制 1-3 个，贴近微信熟人语感，不堆叠不夸张",
      lineBreaks: "短句分行，承接 CTA（关注公众号/加企微/进群/点小程序）单独成行",
      ctaStyle: "先社交转发再公私域承接，强信任弱硬推；引导点赞撬动『朋友在看』关系链，规避诱导分享与硬导流红线",
      ctaExamples: ["点个赞，让更多朋友也能看到", "转发给那个正需要的朋友", "关注我+点公众号名片领完整版", "评论区扣『1』，我把资料发给你"]
    },
    creditMultiplier: 1,
    platformFit: 90
  }
...
export function createInitialState() {
  ...
  return {
    ...
    workspace: {
      ...
      plan: "Studio",
      credits: DEFAULT_TEST_CREDITS,
      monthlyCredits: DEFAULT_TEST_CREDITS,
      locale: "zh-CN"
    },
    brandKit: { ...defaultBrandKit },
    assets: [
...
export function normalizeStateShape(input = {}) {
  ...
  return {
    ...
    rednoteHandoffs: arrayOr(source.rednoteHandoffs, fallback.rednoteHandoffs),
...
export function hasTaskCreditReservation(state, task, options = {}) {
  const reservationId = creditReservationIdFor(task, options);
  return arrayOr(state?.creditReservations).some(reservation => reservation?.id === reservationId);
}

export function releaseTaskCreditsIfReservedInState(state, task, options = {}) {
  const reserveAmount = numericCredit(options.reserveAmount, numericCredit(task?.credits?.estimated, numericCredit(task?.credits?.actual)));
  if (reserveAmount <= 0) return state;
  if (!hasTaskCreditReservation(state, task, options)) return state;
  return settleTaskCreditsInState(
    state,
...
// 从自由文本中识别目标平台。命中专属 token 走对应平台，其余（含抖音/douyin/tiktok 等）一律回退抖音。
// 平台预设见 platformPresets（小红书 / 抖音 / 视频号 / Instagram Reels / YouTube Shorts / Shopify PDP）。
function detectPlatform(text = "") {
  const lower = text.toLowerCase();
  if (text.includes("小红书") || lower.includes("xiaohongshu") || lower.includes("rednote") || lower.includes("xhs")) {
    return "小红书";
  }
  // 视频号：仅匹配清晰 token（视频号 / shipinhao / wechat|weixin|wx channels），避开过短缩写误命中。
  if (text.includes("视频号") || lower.includes("shipinhao") || lower.includes("wechat channel") || lower.includes("weixin channel") || lower.includes("wx channel")) {
    return "视频号";
  }
...
export function creditEstimateTotal(quote) {
  if (Number.isFinite(quote)) return quote;
  if (Number.isFinite(quote?.estimated)) return quote.estimated;
  return 0;
}
...
export function planCreativeTask({ brief, skill, brandKit = defaultBrandKit, variantCount } = {}) {
  ...
  return {
    ...
    skillId: skill.id,
    skillIds: Array.isArray(skill.skillIds) ? skill.skillIds : [skill.id],
