...
export function runCreativeWorkflowWithSkill({ brief, skill, brandKit = defaultBrandKit, variantCount }) {
  ...
  const variants = buildVariants(normalizedBrief, brandKit, skill, variantCount)
    .map(variant => refreshVariantArtifacts(normalizedBrief, variant, skill));
  ...
  const taskArtifacts = buildTaskArtifacts(qa, taskId);
  return {
    id: taskId,
    status: "completed",
    skillId: skill.id,
    ...
    exports: skill.agents.includes("export") ? buildExports(normalizedBrief, variants, skill, taskArtifacts) : [],
    createdAt: now(),
    updatedAt: now()
  };
}
...
export function buildExportRecord(project, variant, platform = "抖音") {
  const files = buildExportFiles({ brief: { productName: project.name, platform }, variant, skill: findSkill(project.skillId) });
  return {
    id: makeId("export"),
