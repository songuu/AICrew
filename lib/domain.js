...
import { createCreditWallet, releaseReservation, reserveCredits, settleReservation } from "./credits.js";
...
export function settleTaskCreditsInState(state, task, options = {}) {
  ...
  const reservationId = options.reservationId || `${task?.id || makeId("task")}:${options.reason || "generation"}`;
  ...
  const final = shouldRelease
    ? releaseReservation(reserved.wallet, reservationId, { idempotencyKey: `release:${reservationId}` })
    : settleReservation(reserved.wallet, reservationId, {
        actualAmount,
  ...
  return {
    ...
    workspace: {
      ...state.workspace,
      credits: final.wallet.available,
      reservedCredits: final.wallet.reserved
    },
    creditReservations: final.wallet.reservations,
    creditReservationLedger: final.wallet.ledger,
    creditLedger: [
      displayCreditEntry({
        task,
        label: displayLabel,
