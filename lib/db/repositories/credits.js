...
export function normalizeCreditWalletFromWorkspace(workspaceId, workspace = {}) {
  ...
  const migratedWallet = loadedWallet.buckets.length === 0 && loadedWallet.availableCredits > 0
    ...
    : loadedWallet;
  if (migratedWallet !== loadedWallet) dirty = true;
  ...
  if (
    wallet.availableCredits !== migratedWallet.availableCredits ||
    wallet.reservedCredits !== migratedWallet.reservedCredits ||
    wallet.lifetimeConsumed !== migratedWallet.lifetimeConsumed ||
    wallet.buckets.length !== migratedWallet.buckets.length ||
    wallet.transactions.length !== migratedWallet.transactions.length
  ) {
    dirty = true;
  }
  return { payload, wallet, dirty };
}
...
async function loadWalletForUpdate(tx, workspaceId) {
  ...
  `;
  return normalizeCreditWalletFromWorkspace(workspaceId, workspace);
}
...
export async function loadCreditWalletOverview(workspaceId = DEFAULT_WORKSPACE_ID) {
  ...
  const [workspace] = await sql`
    select credits, payload from public.aicrew_workspaces where id = ${workspaceId}
  `;
  if (workspace) {
    const { wallet, dirty } = normalizeCreditWalletFromWorkspace(workspaceId, workspace);
    if (!dirty) return serviceResult(wallet, { catalog: getCreditCatalog(), ledger: transactionsForDisplay(wallet) });
  }
  return sql.begin(async tx => {
    const { payload, wallet, dirty } = await loadWalletForUpdate(tx, workspaceId);
    if (dirty) await saveWallet(tx, workspaceId, payload, wallet);
