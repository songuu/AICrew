...
export function createCreditSystemWallet(source = {}) {
  ...
  return {
    id: String(source.id ?? source.walletId ?? source.wallet_id ?? "wallet_default"),
...
export function ensureCreditSystemDefaultGrant(walletInput, input = {}) {
  const wallet = createCreditSystemWallet(walletInput);
  const targetCredits = creditAmount(input.targetCredits ?? input.defaultCredits ?? 0, "targetCredits", { allowZero: true });
  const openingCredits = wallet.availableCredits + wallet.reservedCredits + wallet.lifetimeConsumed;
  const missingCredits = targetCredits - openingCredits;
  if (missingCredits <= 0) return wallet;
  return grantCreditBucket(wallet, {
    amount: missingCredits,
    sourceType: input.sourceType ?? "grant",
    label: input.label ?? "测试默认积分补足",
    bucketId: input.bucketId ?? `${wallet.id}:default-credit-grant:${targetCredits}`,
    idempotencyKey: input.idempotencyKey ?? `grant:${wallet.id}:default-credit-grant:${targetCredits}`,
    priority: input.priority ?? 90,
    createdAt: input.createdAt ?? null,
    metadata: { targetCredits, reason: "testing_default_credits", ...objectOr(input.metadata) }
  }).wallet;
}
...
export function claimDailyRefresh(walletInput, input = {}) {
  const dailyGrant = resolveDailyRefreshGrant(walletInput, input);
  const { wallet, plan, now, day, amount, ageDays, bucketId, idempotencyKey, sourceType, expiresAt } = dailyGrant;
...
export function getDailyCheckInState(walletInput, input = {}) {
  const dailyGrant = resolveDailyRefreshGrant(walletInput, input);
  const { wallet, plan, now, day, amount, ageDays, bucketId, idempotencyKey, sourceType, expiresAt } = dailyGrant;
  const transaction = findTransactionByIdempotency(wallet, idempotencyKey);
  const bucket = wallet.buckets.find(candidate => candidate.id === (transaction?.bucketId ?? bucketId)) ?? null;
  return {
    day,
    planId: plan.id,
    amount,
    ...
    bucketId,
    idempotencyKey,
    checkedIn: Boolean(transaction),
    checkedAt: transaction?.createdAt ?? null,
    transactionId: transaction?.id ?? null,
    expiresAt,
    ageDays,
    serverNow: now.toISOString(),
    remainingTodayCredits: bucket?.remainingAmount ?? 0
  };
}
...
export function buildCreditWalletOverview(walletInput, options = {}) {
  ...
  return {
    ...
    permanentCredits,
    dailyCheckIn: getDailyCheckInState(wallet, options),
    buckets,
...
function resolveDailyRefreshGrant(walletInput, input = {}) {
  const wallet = createCreditSystemWallet(walletInput);
  const plan = findMembershipPlan(input.planId ?? wallet.planId);
  const now = parseDate(input.now, new Date());
