...
export function createCreditWallet(source = {}) {
  ...
  if (source.openingBalance != null) walletInput.openingBalance = source.openingBalance;

  return normalizeCreditWallet(walletInput);
}
...
export function settleReservation(walletInput, reservationId, settleInput = {}) {
  ...
  if (actualAmount > reservedAmount) {
    throw new CreditAccountingError(
      ...
      `Cannot settle ${actualAmount} credits for reservation ${reservation.id}; only ${reservedAmount} credits were reserved.`,
      {
        walletId: wallet.id,
  ...
  const settledAmount = actualAmount;
