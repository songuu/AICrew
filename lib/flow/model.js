...
import { agents, skills, findPlatformPreset, isVideoSkill, findSkill } from "../domain.js";
...
export function skillIdsToFlow(skillIds = [], mode = "auto", brief = null) {
  const selectedSkills = uniqueSkillIds(skillIds)
    .map(id => skills.find(skill => skill.id === id))
    .filter(Boolean);
  if (!selectedSkills.length) return createFlow(mode, brief);
  ...
  return linearFlow(agentIds, mode, brief);
}
...
export function flowToSkill(flow, meta = {}) {
  ...
  const skill = {
    ...
    agents: orderedIds,
    skillIds: Array.isArray(meta.skillIds) ? meta.skillIds : undefined,
    palette: orderedAgents.slice(0, 3).map(agent => agent.accent),
    promise: meta.promise || "由编排图实时生成",
    // 选中预设 skill 时由 meta 透传创作意图，供 AI 文案 / 出图 prompt 注入风格；
    // 纯编排图（无预设）时为空串，prompt 注入按空值跳过，行为与改动前一致。
