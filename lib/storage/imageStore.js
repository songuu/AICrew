import { variantImageRefKey, libraryAssetRefKey } from "../artifacts.js";
...
function libraryKey(id) {
  return libraryAssetRefKey(id);
}

// 落盘前：把 state.assets 内各库素材的 ref（dataURL / https）批量写入同一 imageStore。
// 与 stashVariantImages 共用 store：两者顺序调用即合并写入，库素材走 library:<id> 独立命名空间。
// 返回写入条数。配合 sanitizeStateForStorage 把 base64 移出主 snapshot，避免 snapshot 撑爆又不丢素材。
//
// maxBytes：推 Supabase 路径应传 STASH_UNBOUNDED（无配额），否则大素材在写云前被本地 LRU 驱逐 → 权威源永久丢素材。
export function stashLibraryAssets(state, storage, maxBytes = DEFAULT_IMAGE_QUOTA_BYTES) {
  let store = loadImageStore(storage);
  let count = 0;
  for (const asset of state?.assets || []) {
    if (asset?.id && typeof asset.ref === "string" && asset.ref) {
      store = putRecord(store, libraryKey(asset.id), asset.ref);
      count += 1;
    }
  }
  saveImageStore(store, storage, maxBytes);
...
// 读取后：用 imageStore 回填被剥离的 asset.ref（命中才填，缺失保持原样不报错）。纯函数。
export function rehydrateLibraryAssets(state, storage) {
  const store = loadImageStore(storage);
  const assets = Array.isArray(state?.assets)
    ? state.assets.map(asset => {
        if (!asset?.id || (typeof asset.ref === "string" && asset.ref)) return asset;
        const record = getRecord(store, libraryKey(asset.id));
        return record ? { ...asset, ref: record.url } : asset;
      })
    : state?.assets;
  return { ...state, assets };
}
...
export function rehydrateVariantImages(state, storage) {
  ...
  const fill = variant => {
    ...
    if (!record) return variant;
    const artifacts = Array.isArray(variant.artifacts)
      ? variant.artifacts.map(artifact =>
          artifact?.type === "image" && artifact.status === "ready" && !artifact.url && artifact.refKey === record.id
    ...
    return {
      ...variant,
      ...(variant.imageUrl ? {} : { imageUrl: record.url }),
      ...(artifacts ? { artifacts } : {})
    };
  };
