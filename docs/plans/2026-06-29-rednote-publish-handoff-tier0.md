...
> 研究（`2026-06-29-xiaoji-app-publish-mechanism-handoff.md`）：小鸡AI App「一键发布」底层 = 官方分享交接（分享 SDK + `xhsdiscover://` 深链），用户手动确认。Web 有对等 API（Web Share / Clipboard / URL Scheme / Blob 下载）。本 sprint 把这套合规交接运用进导出中心。
...
| 打开发布器 | 点「打开发布器」 | `<a href="xhsdiscover://post_note/">` | 移动端唤起发布器 |
...
### 改动清单
1. `lib/share/rednote.js`（新，纯函数）：`REDNOTE_PUBLISH_DEEPLINK` 常量、`supportsRednoteHandoff(platform)`、`buildRednoteShareText(variant)→{caption,hashtags,text}`。
...
- `AICrewStudio.jsx`：`oneClickRednotePublish(text,imageFiles)`「一键带稿去发布」——移动端优先 Web Share 带图（文案随附）→否则复制+唤起官方发布器，落地后长按粘贴；`RednoteHandoff` 加主按钮 + 三步引导。
